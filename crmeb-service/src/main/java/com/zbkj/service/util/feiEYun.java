...
//			System.out.println(method5);
    ...
    public String print(MerchantOrder order, MerchantPrint merchantPrint, Merchant merchant){
        ...
        // 组装打印内容
        String content;
        content = "<CB>"+merchant.getName()+"</CB><BR>";
        content += "--------------------------------<BR>";
        content += "订单编号:" + order.getOrderNo()+"\n";
        content += "日   期:" + CrmebDateUtil.dateToStr(order.getCreateTime(), DateConstants.DATE_FORMAT)+"\n";
        content += "姓   名:" + (ObjectUtil.isEmpty(order.getRealName())?"-":order.getRealName())+"\n";
        content += "电   话:" + (ObjectUtil.isEmpty(order.getUserPhone())?"-":order.getUserPhone())+"\n";
        if(order.getShippingType().equals(1)) { // 仅核销订单不打印地址
            content += "地   址:" + order.getUserAddress()+"\n";
        }
        content += "订单备注:"+ order.getUserRemark()+"\n" ;
        content += "--------------------------------<BR>";
        content += "名称　　　　　 单价  数量 金额<BR>";
        content += "--------------------------------<BR>";
        content += printUtil.printFormatGoodsList(orderDetailService.getShipmentByOrderNo(order.getOrderNo()));
        content += "--------------------------------<BR>";
        content += "赠送积分:"+order.getGainIntegral()+"<BR>";
        content += "合计:"+ order.getProTotalPrice()+"元，优惠:"+order.getCouponPrice()+"元<BR>";
        content += "邮费:"+order.getTotalPostage()+"元，抵扣:"+order.getIntegralPrice()+"元<BR>";
        content += "<L>实际支付:"+ order.getPayPrice() +"元</L><BR>";
